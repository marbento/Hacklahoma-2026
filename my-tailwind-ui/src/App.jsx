...
function App() {
  ...
  const trails = [
    ...(hasDetour ? [{ name: 'Detour', background: detourImage, nodes: detourNodes }] : []),
  ...
  const isFinale = currentTrail === trails.length - 1 && currentTile === maxTile
              ...
              <div className="flex-1 overflow-y-auto px-5 pb-6 pt-6">
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 rounded-xl bg-[#fbf8ef] px-2 py-1 shadow-md">
                        ...
                        <span className="text-xs font-bold text-[#3b3528]">{bankedSteps}</span>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="relative h-full">
                        ...
                        {Array.from({ length: currentNodes.length }, (_, i) => {
                      ...
                      >
                        <div className={`relative ${isFinale ? 'finale-glow' : ''}`} style={{ width: '44px', height: '44px' }}>
                          {activeSprite ? (
                            <div
                    ...
                    <details className="group mt-6 rounded-3xl border border-[#d8d0c2] bg-[#fbf8ef]/75 p-4 shadow-[0_16px_30px_rgba(79,70,51,0.18)]" style={{ display: 'none' }}>
                ...
                {activeTab === 'stats' && (
                  <div className="space-y-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-[#7c7666]">Completed Tasks</p>
                      <h1 className="mt-2 text-3xl font-semibold italic text-[#3b3528] font-display">Integrated Task Feed</h1>
                      <p className="mt-1 text-xs text-[#6f6a5c]">Crossed-off work that banked steps</p>
                    </div>
                        ...
                        {completedTasks.map((task) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <span className="flex h-6 w-6 items-center justify-center rounded-full border border-[#d8d0c2] bg-[#f5eee0]/80 text-[11px]">✓</span>
                              <div>
                                <p className="text-sm font-medium line-through text-[#6f6a5c]">{task.title}</p>
                                <p className="text-[10px] text-[#8c8576]">{task.source}</p>
                              </div>
                            </div>
                            <span className="rounded-full bg-[#dfe6cf] px-2 py-1 text-[10px] text-[#6f7758]">+{task.steps} 👣</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="rounded-2xl border border-[#d8d0c2] bg-[#fbf8ef]/55 p-1 shadow-[0_10px_18px_rgba(79,70,51,0.12)] backdrop-blur-sm">
                  <div className="grid grid-cols-4 gap-1">
                    <button
                      ...
                      type="button"
                    >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="stroke-current" strokeLinecap="square" strokeLinejoin="miter">
      ...
      {showFinaleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-6">
          <div className="w-full max-w-sm rounded-3xl border border-[#d8d0c2] bg-[#fbf8ef]/90 p-6 shadow-[0_18px_40px_rgba(79,70,51,0.2)] backdrop-blur-sm">
            <p className="text-xs uppercase tracking-[0.3em] text-[#7c7666]">Trek Complete</p>
            <h2 className="mt-2 text-2xl font-semibold italic font-display text-[#3b3528]">Ready for a new trek?</h2>
            <p className="mt-2 text-sm text-[#6f6a5c]">You reached the summit. Want to reset and start again?</p>

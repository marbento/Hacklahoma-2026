...
import { useNudge } from "../../hooks/useDashboard";
import { useGoals } from "../../hooks/useGoals";
const DeviceActivity = require("react-native-device-activity");

type AuthStatus = "notDetermined" | "denied" | "approved";

function getNudgeMessage(goal: string | null, assignment: string | null): string {
  const parts: string[] = [];
  if (goal) parts.push('Remember, you wanted to: "' + goal + '"');
  if (assignment) parts.push("You also have coming up: " + assignment);
  if (parts.length === 0) parts.push("You've been spending a lot of time here.\n\nConsider doing something productive.");
...
export default function HomeScreen() {
  ...
  const topGoal = goals.length > 0 ? goals[0].title : null;
  const upcomingAssignment = nudge?.upcoming_assignment || null;
  ...
  useEffect(() => {
    if (authStatus === "approved") {
      ScreenTimeService.updateNudge(
        ...
        getNudgeMessage(currentGoal, upcomingAssignment)
      );
    }
  }, [authStatus, currentGoal, upcomingAssignment, nudge]);
  ...
  const handleBlock = useCallback(() => {
    if (!selection) { Alert.alert("Select Apps", "Pick apps first."); return; }
    ...
    setIsBlocking(true);
  }, [selection, currentGoal, upcomingAssignment, nudge]);
  ...
  const handleStartMonitoring = useCallback(() => {
    if (!selection) { Alert.alert("Select Apps", "Pick apps first."); return; }
    ScreenTimeService.updateNudge("Time check ⏱️", "You hit your " + timeLimit + "-min limit.\n\n" + getNudgeMessage(currentGoal, upcomingAssignment));
    ScreenTimeService.startMonitoring(selection, timeLimit);
    setIsMonitoring(true);
    Alert.alert("Trail is watching 👁️", "After " + timeLimit + " min of use you will get a nudge.");
  }, [selection, timeLimit, currentGoal, upcomingAssignment]);
    ...
    return (
      <SafeAreaView style={st.container}>
        <View style={st.centered}>
          ...
          <Text style={st.description}>Trail helps you stay focused by giving you a friendly reminder when you open distracting apps.</Text>
          <TouchableOpacity style={st.primaryBtn} onPress={handleAuth}>
            <Text style={st.primaryBtnText}>{authStatus === "denied" ? "Open Settings" : "Enable Screen Time"}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={st.container}>
      <ScrollView contentContainerStyle={st.scroll}>
        ...
        <View style={st.card}>
          ...
          {currentGoal ? (
            <View>
              ...
              {upcomingAssignment && (
                <Text style={st.assignmentHint}>📚 Next up: {upcomingAssignment}{nudge?.assignment_due ? ` (${nudge.assignment_due})` : ""}</Text>
              )}
            ...
            <View>
              <Text style={st.cardHint}>Set a focus so Trail can remind you</Text>
              <TextInput style={st.goalInput} placeholder="e.g., Finish Calc 2 HW by Friday" placeholderTextColor="#555" value={goalInput} onChangeText={setGoalInput} onSubmitEditing={handleSaveGoal} returnKeyType="done" />
              ...
              {goals.length > 0 && (
                <View style={{ marginTop: 12 }}>
                  <Text style={{ color: "#666", fontSize: 12, marginBottom: 6 }}>Or pick from your goals:</Text>
        ...
        <View style={st.pickerContainer}>
          <DeviceActivity.DeviceActivitySelectionView style={st.picker} familyActivitySelection={selection} onSelectionChange={(e: any) => setSelection(e.nativeEvent.familyActivitySelection)} />
        </View>
        ...
        <View style={st.timeLimitRow}>
          {[15, 30, 60, 120].map((m) => (
            <TouchableOpacity key={m} style={[st.timePill, timeLimit === m && st.timePillActive]} onPress={() => setTimeLimit(m)}>
              <Text style={[st.timePillText, timeLimit === m && st.timePillTextActive]}>{m < 60 ? m + "m" : m / 60 + "h"}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {isMonitoring && (
          <View style={st.statusCard}>
            ...
            <Text style={st.statusText}>Trail is active. After {timeLimit} min of use you will see a reminder.</Text>
          </View>
        )}

        {/* Nudge redirect links */}
        {nudge?.redirect_links && nudge.redirect_links.length > 0 && (
          <View style={{ marginTop: 20 }}>
            <Text style={st.sectionTitle}>Quick Links</Text>
            {nudge.redirect_links.map((link, i) => (
              <TouchableOpacity key={i} style={st.linkCard} onPress={() => {/* Linking.openURL(link.url) */}}>
...
const st = StyleSheet.create({
  ...
  assignmentHint: { fontSize: 13, color: "#888", marginTop: 8 },
  ...
  goalPick: { backgroundColor: "#222", borderRadius: 8, padding: 10, marginVertical: 3 },
